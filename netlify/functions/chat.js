...
> 💡 **အရေးကြီးဆုံးရွှေရောင်စည်းမျဉ်း:** သင်၏ KPay/Wave Money OTP (ဂဏန်း ၆ လုံး) နှင့် PIN နံပါတ်ကို ဘဏ်ဝန်ထမ်းအပါအဝင် ဘယ်သူ့ကိုမှ လုံးဝ (လုံးဝ) မပြောပါနှင့်။`;
  ...
  try {
    ...
    for (let i = 0; i < MODELS.length; i++) {
      ...
      console.error(`Gemini [${model}] HTTP ${res.status} — ${errData?.error?.message || 'no message'} | code: ${errData?.error?.code || '?'}`);

      // 400 = bad request (wrong format/model name), no point retrying same payload
      if (res.status === 400) {
        console.error(`Gemini 400 details:`, JSON.stringify(errData?.error));
    ...
    // All models failed
    if (!data) {
      console.error(`All Gemini models failed. Tried: ${MODELS.join(', ')} — last HTTP status: ${lastStatus}`);
      await logToSupabase(supabaseUrl, supabaseKey, {
        session_id: sessionId || 'anon', role: 'error',
        message: `All models failed (${MODELS.join(',')}), last status: ${lastStatus}`, ip_hash: ipHash, created_at: new Date().toISOString()
      });
      return { statusCode: 200, headers, body: JSON.stringify({ reply: FALLBACK_REPLY, cached: true }) };
    }
